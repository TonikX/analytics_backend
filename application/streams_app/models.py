...
class Classroom(models.Model):
    ...
    is_fake = models.BooleanField(
        verbose_name="настоящая ли аудитория", blank=True, null=True
    )
    ...
    departments = models.ManyToManyField(
        "workprogramsapp.Department", verbose_name="Факультеты", blank=True
    )
    group = models.ManyToManyField(
        "ClassroomGroup", blank=True, verbose_name="группа аудитория"
    )
...
class Building(models.Model):
    ...
    short_name = models.CharField(
        max_length=1024, verbose_name="Сокращенное наименование здания"
    )
    is_external = models.BooleanField(
        verbose_name="внешнее здание", blank=True, null=True
    )
