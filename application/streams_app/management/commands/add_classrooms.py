...
from workprogramsapp.models import Department
    ...
    def handle(self, *args, **options):
        ...
        for class_json in data["list"]:
            ...
            if building_part_json:
                building_part, created = BuildingPart.objects.get_or_create(
                    service_id=building_part_json["id"]
                )

                if created:
                    building_part.address = building_part_json["address"]
                    building_part.version = building_part_json["version"]
                    building_part.updated_at = self.convert_java_datetime(
                        building_part_json["updatedAt"]
                    )
                    ...
                    if building_json:
                        building, created = Building.objects.get_or_create(
                            service_id=building_json["id"]
                        )
                        if created:
                            building.version = building_json["version"]
                            building.updated_at = self.convert_java_datetime(
                                building_json["updatedAt"]
                            )
            ...
            for dep_json in class_json["departments"]:
                try:
                    department = Department.objects.get(
                        mini_titile=dep_json["shortName"]
                    )
                ...
                department.updated_at = self.convert_java_datetime(
                    dep_json["updatedAt"]
                )
                department.save()
