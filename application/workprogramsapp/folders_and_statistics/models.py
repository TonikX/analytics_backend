...
class WorkProgramInFolder(models.Model):
    ...
    work_program = models.ForeignKey(
        "WorkProgram", verbose_name="Рпд в папке", on_delete=models.CASCADE
    )
    work_program_rating = models.PositiveSmallIntegerField(
        choices=RATING_CHOICES,
        verbose_name="Важность рпд",
        blank=True,
        null=True,
        default=0,
    )
    comment = models.CharField(
        max_length=10240, verbose_name="Комментарий", blank=True, null=True
    )
...
class AcademicPlanInFolder(models.Model):
    ...
    academic_plan = models.ForeignKey(
        "AcademicPlan", verbose_name="УП в папке", on_delete=models.CASCADE
    )
    academic_plan_rating = models.PositiveSmallIntegerField(
        choices=RATING_CHOICES,
        verbose_name="Важность уп",
        blank=True,
        null=True,
        default=0,
    )
...
class DisciplineBlockModuleInFolder(models.Model):
    ...
    block_module = models.ForeignKey(
        "DisciplineBlockModule", verbose_name="модуль в папке", on_delete=models.CASCADE
    )
    module_rating = models.PositiveSmallIntegerField(
        choices=RATING_CHOICES,
        verbose_name="Важность модуля",
        blank=True,
        null=True,
        default=0,
    )
...
class IndividualImplementationAcademicPlanInFolder(models.Model):
    ...
    individual_implementation_of_academic_plan = models.ForeignKey(
        ...
        verbose_name="траектория в папке",
        on_delete=models.CASCADE,
    )
    route_rating = models.PositiveSmallIntegerField(
        choices=RATING_CHOICES,
        verbose_name="Важность траектории",
        blank=True,
        null=True,
        default=0,
    )
...
class Folder(models.Model):
    ...
    description = models.CharField(
        max_length=1024, verbose_name="Описание папки", blank=True, null=True
    )
