...
from workprogramsapp.isu_merge.academic_plan_update_2023.academic_plan_excel_creator import (
    AcademicPlanExcelCreator,
)
...
from workprogramsapp.isu_merge.academic_plan_update_2023.academic_plan_update_processor import (
    AcademicPlanUpdateProcessor as AcademicPlanUpdateProcessor_2023,
)
...
from workprogramsapp.serializers import (
    AcademicPlanUpdateConfigurationSerializer,
    AcademicPlanUpdateLogSerializer,
...
class AcademicPlanUpdateConfigurationView(ListAPIView):
    ...
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ["academic_plan_id", "academic_plan_title", "updated_date_time"]
    ordering_fields = ["academic_plan_id", "academic_plan_title", "updated_date_time"]
    permission_classes = [permissions.IsAuthenticated]
...
class AcademicPlanUpdateSchedulerConfigurationView(RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        queryset = AcademicPlanUpdateSchedulerConfiguration.objects.all()
        serializer = AcademicPlanUpdateSchedulerConfigurationSerializer(
...
class FileUploadAPIView(APIView):
    ...
    def post(self, request):
        ...
        for i in list(data.index.values):
            try:
                ...
                print(
                    "dsssss",
                    AcademicPlan.objects.filter(ap_isu_id=int(data["ИД_УП"][i])),
                )
    ...
    def process_disciplines_block(
        ...
        for educational_module in educational_modules:
            current_module = disciplines_block[
                disciplines_block[field_to_iterate_over] == educational_module
            ]
            ...
            for academic_plan_id, row_ids in groupby_result.groups.items():
                ...
                for version in educational_module_versions:
                    ...
                    if FileUploadAPIView.check_if_modules_are_the_same(
                        module1, module2
                    ):
            ...
            for module_version in educational_module_versions:
                discipline_block_module = DisciplineBlockModule(
                    name=educational_module_name, order=module_order
                )
                ...
                for academic_plan_id, row_ids in module_version.items():
                    for row_id in row_ids:
                        cls.educational_module_by_row_id[row_id] = (
                            discipline_block_module.pk
                        )

    @staticmethod
    def check_if_modules_are_the_same(
        module1: pandas.DataFrame, module2: pandas.DataFrame
    ) -> bool:
        ...
        for i in range(sorted_module1.shape[0]):
            ...
            if not FileUploadAPIView.check_if_disciplines_are_the_same(
    ...
    @staticmethod
    def check_if_disciplines_are_the_same(
        discipline1: pandas.core.series.Series, discipline2: pandas.core.series.Series
    ) -> bool:
        ...
        if (
            discipline1["ЛЕК_В_СЕМЕСТРАХ"] != discipline2["ЛЕК_В_СЕМЕСТРАХ"]
            or discipline1["ЛАБ_В_СЕМЕСТРАХ"] != discipline2["ЛАБ_В_СЕМЕСТРАХ"]
            or discipline1["ПРАК_В_СЕМЕСТРАХ"] != discipline2["ПРАК_В_СЕМЕСТРАХ"]
        ):
            return False
        ...
        for i in list(data.index.values):
            try:
                ...
                wp_obj.lecture_hours_v2 = watchmaker(
                    [
                        float(x)
                        for x in (data["ЛЕК_В_СЕМЕСТРАХ"][i]).strip("()").split(",")
                    ],
                    [
                        float(x)
                        for x in (data["ЗЕ_В_СЕМЕСТРАХ"][i]).strip("()").split(",")
                ...
                wp_obj.lab_hours_v2 = watchmaker(
                    [
                        float(x)
                        for x in (data["ЛАБ_В_СЕМЕСТРАХ"][i]).strip("()").split(",")
                    ],
                    [
                        float(x)
                        for x in (data["ЗЕ_В_СЕМЕСТРАХ"][i]).strip("()").split(",")
                ...
                if StructuralUnit.objects.filter(
                    title=str(data["ИСПОЛНИТЕЛЬ_ДИС"][i].strip())
                ):
                    st_unit = StructuralUnit.objects.filter(
                        title=data["ИСПОЛНИТЕЛЬ_ДИС"][i].strip()
                    )[0]
                    ...
                    StructuralUnit.objects.create(
                        title=data["ИСПОЛНИТЕЛЬ_ДИС"][i].strip(),
                        isu_id=int(data["ИД_ИСПОЛНИТЕЛЯ_ДИС"][i]),
                    )
                ...
                if DisciplineBlockModule.objects.filter(
                    ...
                    mdb = DisciplineBlockModule(
                        name=(data["МОДУЛЬ"][i].strip()), order=o
                    )
                    mdb.save()
                    mdb.descipline_block.add(db)
...
class SendWorkProgramToISU(APIView):
    ...
    def post(self, request):
        ...
        if not wp.structural_unit:
            return Response(
                data={"error": "У РПД нету структурного подразделения"}, status=500
            )
        ...
        if created_id:
            ...
            return Response(
                data={
                    "error": "error when creating wp",
                    "isu_response": response_data["error_message"],
                },
...
class SendPracticeToISU(APIView):
    ...
    def post(self, request):
        ...
        if not practice.structural_unit:
            return Response(
                data={"error": "У практики нету структурного подразделения"}, status=500
            )
        isu_logger = IsuService(
            IsuUser(settings.ISU["ISU_CLIENT_ID"], settings.ISU["ISU_CLIENT_SECRET"])
        )
        ...
        if created_id:
            ...
            practice.save()
            return Response(
                data={"practice_created_id": created_id}, status=status_response
            )
        else:
            status_response = 500
            return Response(
                data={"error": "error when creating practice"}, status=status_response
            )
...
class SendGIAToISU(APIView):
    ...
    def post(self, request):
        ...
        if not gia.structural_unit:
            return Response(
                data={"error": "У ГИА нету структурного подразделения"}, status=500
            )
        isu_logger = IsuService(
            IsuUser(settings.ISU["ISU_CLIENT_ID"], settings.ISU["ISU_CLIENT_SECRET"])
        )
        ...
        if created_id:
            ...
            status_response = 500
            return Response(
                data={"error": "error when creating gia"}, status=status_response
            )
...
def IsuModulesDuplicates(request):
    isu_modules = (
        DisciplineBlockModuleInIsu.objects.values("isu_id")
        .annotate(Count("id"))
        .order_by()
    ...
    for isu_module in isu_modules:
        duplicate = DisciplineBlockModuleInIsu.objects.filter(
            isu_id=isu_module["isu_id"]
        )
        ...
        if duplicate.count() == duplicate.filter(module=duplicate_module).count():
            ...
            for d in duplicate:
                imp = ImplementationAcademicPlan.objects.get(
                    academic_plan=d.academic_plan
                )
                duplicate_list.append(
                    {"isu_id": d.isu_id, "our_id": d.module.id, "ap_id": imp.ap_isu_id}
                )
