...
from workprogramsapp.isu_merge.academic_plan_update.academic_plan_update_aspect import (
...
class AcademicPlanUpdateProcessor:
    ...
    def __process_discipline__(
        ...
        def structural_unit(isu_academic_plan_discipline_json):
            if StructuralUnit.objects.filter(
                title=str(isu_academic_plan_discipline_json["discipline_doer"].strip())
            ):
                st_unit = StructuralUnit.objects.filter(
                    title=isu_academic_plan_discipline_json["discipline_doer"].strip()
                )[0]
                st_unit.isu_id = int(
                ...
                StructuralUnit.objects.create(
                    title=isu_academic_plan_discipline_json["discipline_doer"].strip(),
                    isu_id=int(isu_academic_plan_discipline_json["discipline_doer_id"]),
                )
                st_unit = StructuralUnit.objects.get(
                    title=isu_academic_plan_discipline_json["discipline_doer"].strip(),
                    isu_id=int(isu_academic_plan_discipline_json["discipline_doer_id"]),
                )
        ...
        consultation = AcademicPlanUpdateUtils.get_consultation(
            isu_academic_plan_discipline_json
        )

        work_program_object.number_of_semesters = int(semesters([float(x) for x in ze]))

        work_program_object.lecture_hours_v2 = watchmaker(
            [float(x) for x in lec], [float(x) for x in ze]
        )

        work_program_object.practice_hours_v2 = watchmaker(
            [float(x) for x in prac], [float(x) for x in ze]
        )

        work_program_object.consultation_v2 = watchmaker(
            [float(x) for x in consultation], [float(x) for x in ze]
        )

        work_program_object.lab_hours_v2 = watchmaker(
            [float(x) for x in lab], [float(x) for x in ze]
        )

        work_program_object.srs_hours_v2 = watchmaker(
            [
                float(x)
                for x in AcademicPlanUpdateUtils.set_srs(
                    AcademicPlanUpdateUtils.get_srs(ze, lec, prac, lab),
                    ze,
                    lec,
                    prac,
                    lab,
        ...
        work_program_object.discipline_code = str(
            isu_academic_plan_discipline_json["disc_id"]
        )
        if (
            isu_academic_plan_discipline_json["discipline_doer"]
            and isu_academic_plan_discipline_json["discipline_doer_id"] is not None
        ):
            work_program_object.structural_unit = structural_unit(
                isu_academic_plan_discipline_json
            )
        work_program_object.save()
    ...
    def __process_academic_plan__(
        implementation_academic_plan_object, isu_academic_plan_json, field_of_study
    ):
        if implementation_academic_plan_object is not None:
            implementation_academic_plan_object.op_isu_id = int(
                isu_academic_plan_json["edu_program_id"]
            )
            implementation_academic_plan_object.ap_isu_id = int(
                isu_academic_plan_json["id"]
            )
            implementation_academic_plan_object.ns_id = int(
                isu_academic_plan_json["ns_id"]
            )
            implementation_academic_plan_object.title = isu_academic_plan_json[
                "edu_program_name"
            ]
    ...
    def __process_block_module__(
        ...
        if discipline_block_module_object is not None:
            ...
            discipline_block_module_object = DisciplineBlockModule(
                ...
                order=AcademicPlanUpdateUtils().get_module_order(
                    isu_academic_plan_block_module_json
                ),
            )
    ...
    def __process_linked_data__(
        ...
        if (
            ...
            old_work_program_change_in_discipline_block_module = (
                WorkProgramChangeInDisciplineBlockModule.objects.get(
                    discipline_block_module=discipline_block_module_object,
                    change_type=option,
                    subject_code=AcademicPlanUpdateUtils.num_to_int(
                        isu_academic_plan_discipline_json["plan_order"],
            ...
            work_program_change_in_discipline_block_module.credit_units = (
            ...
            work_program_change_in_discipline_block_module.subject_code = (
        ...
        for plan_id in academic_plans_ids:
            try:
                ...
                if isu_academic_plan_json is not None:
                    self.__update_disciplines__(
                        old_academic_plan, isu_academic_plan_json
                    )

                    field_of_study = self.__process_field_of_study__(
                        isu_academic_plan_json
                    )

                    academic_plan = self.__process_academic_plan__(
                        isu_academic_plan_json, field_of_study
                    )
                    block_to_del_ids = []
                    for block in isu_academic_plan_json["disciplines_blocks"]:
                        ...
                        for module in block["discipline_modules"]:
                            ...
                            block_modules_to_del_ids.append(
                                discipline_block_module_object.id
                            )
                            disciplines_for_del_in_module = []
                            work_program_change_in_discipline_block_modules_not_for_del = (
                                []
                            )
                            for isu_academic_plan_discipline_json in module[
                                "disciplines"
                            ]:
                                ...
                                disciplines_for_del_in_module.append(
                                    work_program_object.id
                                )
                            self.__del_work_program_in_field_of_study__(
                                discipline_block_module_object,
                                disciplines_for_del_in_module,
                            )
                            self.__del_old_wpcbms_by_module__(
                                discipline_block_module_object,
                                work_program_change_in_discipline_block_modules_not_for_del,
                            )
                        # print(block_modules_to_del_ids)
                        self.__del_block_modules__(
                            block_modules_to_del_ids,
                            isu_academic_plan_json,
                            discipline_block_object,
                        )
