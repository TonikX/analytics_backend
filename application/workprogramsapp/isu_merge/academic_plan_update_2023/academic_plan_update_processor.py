...
from workprogramsapp.isu_merge.academic_plan_update_2023.isu_service import (
    IsuService,
    IsuUser,
)
...
class AcademicPlanUpdateProcessor:
    ...
    def __del_disciplines_ids_by_academic_plan__(academic_plan_id, new_disciplines_ids):
        wcbms = (
            ...
            .exclude(work_program__discipline_code__in=new_disciplines_ids)
            .delete()
        )
    ...
    def __update_disciplines__(self, old_academic_plan, isu_academic_plan_json):
        old_disciplines_ids = self.__get_disciplines_ids_by_academic_plan__(
            old_academic_plan.ap_isu_id
        )
        new_disciplines_ids = (
            AcademicPlanUpdateUtils.get_disciplines_ids_from_academic_plan_json(
        ...
        to_del = set(
            map(int, [float(i[0]) for i in [i for i in old_disciplines_ids if i != ""]])
        ) - set(map(int, new_disciplines_ids))

        self.__del_disciplines_ids_by_academic_plan__(
            old_academic_plan.ap_isu_id, new_disciplines_ids
        )
        # for wp in to_del:
    ...
    @staticmethod
    def __process_english__(
        isu_academic_plan_json, isu_academic_plan_discipline_json, module_object
    ):

        work_program_object = WorkProgram.objects.get(
        ...
        def choose_department(isu_academic_plan_discipline_json):
            ...
            department_by_id = StructuralUnit.objects.filter(
                isu_id=department_obj["id"]
            )
            if department_by_id.exists():
                department = department_by_id.first()
            else:
                department_by_name = StructuralUnit.objects.filter(
                    title=department_obj["name"]
                )
        ...
        discipline_contents = list(
            isu_academic_plan_discipline_json["contents"].values()
        )[0]
        sorted_discipline_contents = sorted(
            discipline_contents, key=lambda x: x["semester"]
        )
        wp_in_cb_not_for_del = []
        field_of_study_not_for_del = []
        replaceable = isu_academic_plan_discipline_json["replaceable"]
        option = "Required" if not replaceable else "Optionally"
        for sem, sem_dict in enumerate(sorted_discipline_contents):
            if WorkProgramChangeInDisciplineBlockModule.objects.filter(
                work_program=work_program_object,
                discipline_block_module=module_object,
                semester_start=[sem + 1],
            ).exists():
                wp_in_cb = WorkProgramChangeInDisciplineBlockModule.objects.filter(
                    work_program=work_program_object,
                    discipline_block_module=module_object,
                    semester_start=[sem + 1],
                )[0]
                if WorkProgramInFieldOfStudy.objects.filter(
                    ...
                    wp_in_fs = WorkProgramInFieldOfStudy.objects.create(
                        work_program=work_program_object,
                        work_program_change_in_discipline_block_module=wp_in_cb,
                    )

            else:
                wp_in_cb = WorkProgramChangeInDisciplineBlockModule.objects.create(
                    discipline_block_module=module_object,
                    semester_start=[sem + 1],
                    change_type=option,
                )
            ...
            wp_in_cb_not_for_del.append(wp_in_cb.id)
            field_of_study_not_for_del.append(wp_in_fs.id)

        AcademicPlanUpdateProcessor.__del_work_program_in_field_of_study_by_ids__(
            module_object, field_of_study_not_for_del, isu_academic_plan_json
        )
        AcademicPlanUpdateProcessor.__del_old_wpcbms__(
            module_object, wp_in_cb_not_for_del
        )
        ...
        for sem, sem_dict in enumerate(sorted_discipline_contents):
            ...
            for type_dict in sem_dict["activities"]:
                ...
                if wt_id == 1:
                    ...
                    cerf = CertificationEvaluationTool(
                        type=certification_types[wt_id], semester=sem + 1
                    )
        ...
        work_program_object.language = isu_academic_plan_discipline_json[
            "langCode"
        ].lower()
        work_program_object.structural_unit = choose_department(
        ...
        work_program_object.number_of_semesters = 1

        work_program_object.save()

        for cerf_to_connect in cerf_list:
            is_cerf_exist = CertificationEvaluationTool.objects.filter(
                work_program=work_program_object,
    ...
    def __process_discipline__(
        ...
        work_program_object.ze_v_sem = process_hours(ze_v_sem)
        work_program_object.contact_hours_v2 = process_hours(contact_hours)
        work_program_object.number_of_semesters = isu_academic_plan_discipline_json[
            "disciplineDuration"
        ]
    ...
    def __process_academic_plan__(isu_academic_plan_json):
        ...
        if AcademicPlan.objects.filter(
            ...
            academic_plan_object = AcademicPlan.objects.get(
                ap_isu_id=int(isu_academic_plan_json["id"])
            )
    ...
    def __process_discipline_block__(
    ...
    def __del_block_modules__(
        ...
        bm = (
            ...
            .exclude(id__in=block_modules_to_del_ids)
            .delete()
        )

    @staticmethod
    def __del_block_modules__by__father__module__(
        block_modules_not_for_del_ids,
        discipline_block_module_object,
        isu_academic_plan_block_module_json,
        isu_academic_plan_json,
        father_module_json,
    ):
        bms = DisciplineBlockModule.objects.filter(
    ...
    @staticmethod
    def __del_father_block_modules__(
        block_modules_not_for_del_ids,
        discipline_block_object,
        isu_academic_plan_json,
    ):
        ...
        bms = DisciplineBlockModule.objects.filter(
            descipline_block__academic_plan=AcademicPlan.objects.filter(
                ap_isu_id=isu_academic_plan_json["id"]
            )[0]
        ).exclude(id__in=block_modules_not_for_del_ids)
        for bm in bms:
    ...
    def __process_block_module__(
        ...
        discipline_block_module_object.selection_parametr = (
            ", ".join([str(el) for el in isu_academic_plan_block_module_json["rules"]])
            if isu_academic_plan_block_module_json["rules"]
            else None
        )
        discipline_block_module_object.save()
    ...
    def __process_practice_linked_data__(
        discipline_block_module_object,
        practice_object,
        isu_academic_plan_discipline_json,
        isu_academic_plan_json,
        ...
        if WorkProgramChangeInDisciplineBlockModule.objects.filter(
            ...
            list_of_start_terms = [
                int(sem) for sem in isu_academic_plan_discipline_json["contents"]
            ]
            old_work_program_change_in_discipline_block_module.semester_start = (
                list_of_start_terms
            )
            old_work_program_change_in_discipline_block_module.save()
            work_program_change_in_discipline_block_module = copy.deepcopy(
            ...
            if PracticeInFieldOfStudy.objects.filter(
                ...
                practice_in_field_of_study = copy.deepcopy(
            ...
            work_program_change_in_discipline_block_module.discipline_block_module = (
                discipline_block_module_object
            )
            list_of_start_terms = [
                int(sem) for sem in isu_academic_plan_discipline_json["contents"]
            ]
            work_program_change_in_discipline_block_module.semester_start = (
                list_of_start_terms
            )
            work_program_change_in_discipline_block_module.save()
    ...
    @staticmethod
    @AcademicPlanUpdateAspect.linked_data_changes_aspect
    def __process_gia_linked_data__(
        discipline_block_module_object,
        gia_object,
        isu_academic_plan_discipline_json,
        isu_academic_plan_json,
        ...
        if WorkProgramChangeInDisciplineBlockModule.objects.filter(
            ...
            work_program_change_in_discipline_block_module.save()
            work_program_change_in_discipline_block_module.gia.add(gia_object)
            work_program_change_in_discipline_block_module.save()
            print(work_program_change_in_discipline_block_module)
            if GiaInFieldOfStudy.objects.filter(
                work_program_change_in_discipline_block_module=work_program_change_in_discipline_block_module,
    ...
    def __process_linked_data__(
        ...
        if WorkProgramChangeInDisciplineBlockModule.objects.filter(
            ...
            if WorkProgramInFieldOfStudy.objects.filter(
                ...
                work_program_in_field_of_study = copy.deepcopy(
            ...
            work_program_change_in_discipline_block_module.save()
            work_program_change_in_discipline_block_module.work_program.add(
                work_program_object
            )
            work_program_change_in_discipline_block_module.save()
            print(work_program_change_in_discipline_block_module)
    ...
    def __process_work_program_id_str_up_for_isu__(
        work_program_id_str_up_for_isu_object,
        ...
        if work_program_id_str_up_for_isu_object is None:
            ...
            work_program_id_str_up_for_isu_object.number = (
                AcademicPlanUpdateUtils.num_to_int(
                    isu_academic_plan_discipline_json["plan_order"],
                    isu_academic_plan_discipline_json["discipline_name"],
                )
            )
            work_program_id_str_up_for_isu_object.dis_id = (
                isu_academic_plan_discipline_json["dis_id"]
            )
            work_program_id_str_up_for_isu_object.ze_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.get_ze(isu_academic_plan_discipline_json)
                )
            )
            work_program_id_str_up_for_isu_object.lec_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.get_lec(isu_academic_plan_discipline_json)
                )
            )
            work_program_id_str_up_for_isu_object.prak_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.get_prac(isu_academic_plan_discipline_json)
                )
            )
            work_program_id_str_up_for_isu_object.lab_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.get_lab(isu_academic_plan_discipline_json)
                )
            )
            work_program_id_str_up_for_isu_object.ekz_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
            ...
            work_program_id_str_up_for_isu_object.zach_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.set_control(
            ...
            work_program_id_str_up_for_isu_object.dif_zach_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.set_control(
            ...
            work_program_id_str_up_for_isu_object.kp_v_sem = (
                AcademicPlanUpdateUtils.ze_to_format(
                    AcademicPlanUpdateUtils.set_control(
            ...
            for zun in Zun.objects.filter(
                wp_in_fs_saved_fk_id_str_up=int(
                    isu_academic_plan_discipline_json["str_up_id"]
                )
            ):
    ...
    def __del_work_program_in_field_of_study__(
        discipline_block_module, new_disciplines_ids, isu_academic_plan_json
    ):
        ...
        wcbms = WorkProgramInFieldOfStudy.objects.filter(
        ...
        for wcbm in wcbms:
            wcbm.backup_module = DisciplineBlockModule.objects.get(
                change_blocks_of_work_programs_in_modules__zuns_for_cb=wcbm
            )
            wcbm.backup_ap = AcademicPlan.objects.filter(
                ap_isu_id=isu_academic_plan_json["id"]
            )[0]
    ...
    @staticmethod
    def __del_work_program_in_field_of_study_by_ids__(
        discipline_block_module, wp_in_fs_not_for_del, isu_academic_plan_json
    ):
        wcbms = WorkProgramInFieldOfStudy.objects.filter(
            work_program_change_in_discipline_block_module__discipline_block_module=discipline_block_module
        ).exclude(id__in=wp_in_fs_not_for_del)
        for wcbm in wcbms:
            wcbm.backup_module = DisciplineBlockModule.objects.get(
                change_blocks_of_work_programs_in_modules__zuns_for_cb=wcbm
            )
    ...
    @staticmethod
    def __del_practice_in_field_of_study__(
        discipline_block_module, new_disciplines_ids, isu_academic_plan_json
    ):
        pfos = PracticeInFieldOfStudy.objects.filter(
            work_program_change_in_discipline_block_module__discipline_block_module=discipline_block_module
        ...
        for pfo in pfos:
            ...
            pfo.backup_ap = AcademicPlan.objects.filter(
                ap_isu_id=isu_academic_plan_json["id"]
            )[0]
    ...
    @staticmethod
    def __del_gia_in_field_of_study__(
        discipline_block_module, new_disciplines_ids, isu_academic_plan_json
    ):
        pfos = GiaInFieldOfStudy.objects.filter(
            work_program_change_in_discipline_block_module__discipline_block_module=discipline_block_module
        ).exclude(gia__id__in=new_disciplines_ids)
        for pfo in pfos:
            pfo.backup_module = DisciplineBlockModule.objects.get(
                change_blocks_of_work_programs_in_modules__zuns_for_cb_for_practice=pfo
            )
    ...
    @staticmethod
    def __del_old_wpcbms_by_module__(
        discipline_block_module_object,
        ...
        wcbms = (
            ...
            .exclude(
                discipline_block_module=discipline_block_module_object,
                work_program__id__in=work_program_change_in_discipline_block_modules_not_for_del,
            )
            .delete()
        )

    @staticmethod
    def __del_old_wpcbms__(
        discipline_block_module_object,
        work_program_change_in_discipline_block_modules_not_for_del,
    ):
        wcbms = WorkProgramChangeInDisciplineBlockModule.objects.filter(
            discipline_block_module=discipline_block_module_object
        ).exclude(id__in=work_program_change_in_discipline_block_modules_not_for_del)
        wcbms.delete()

    @staticmethod
    def recursion_module_updater(
        module,
        isu_academic_plan_json,
        ...
        if module["type"] == "module":
            ...
            """
            discipline_block_module_object = AcademicPlanUpdateProcessor.__process_block_module__(
                module,
                isu_academic_plan_json,
                # discipline_block_object,
                father_module,
            ...
            if discipline_block_object is not None:
                ...
                discipline_block_module_object.descipline_block.add(
                    discipline_block_object
                )
                discipline_block_module_object.save()
                discipline_block_object = None
                ...
                if str(father_module.get("id")) == new_id:
                    new_id = None
                if new_id:
                    discipline_block_module_object.father_module.add(
                        DisciplineBlockModule.objects.filter(
            ...
            for children_module_dict in module["children"]:
                ...
                children_module, wpcbm = (
                    AcademicPlanUpdateProcessor.recursion_module_updater(
                        module, isu_academic_plan_json, None, father_module
                    )
                )
                if children_module_dict["type"] == "module":
                    modules_not_for_del.append(children_module.id)
                if children_module_dict["type"] == "discipline":
                    if children_module_dict["id"] != 29867:
                        work_program_change_in_discipline_block_modules_not_for_del.append(
                            wpcbm.id
                        )
                    if "practice" in children_module_dict["rpdUrl"]:
                        practices_not_for_del.append(children_module.id)
                    elif (
                        "gia" in children_module_dict["rpdUrl"]
                        or "Блок 3. ГИА" in children_module_dict["blockName"]
                    ):
                        gia_not_for_del.append(children_module.id)
                    else:
            ...
            if module["blockName"] == "Блок 2. Практика" and practices_not_for_del:
                AcademicPlanUpdateProcessor.__del_practice_in_field_of_study__(
                    discipline_block_module_object,
                    practices_not_for_del,
                    isu_academic_plan_json,
                )
            if module["blockName"] == "Блок 3. ГИА":
                AcademicPlanUpdateProcessor.__del_gia_in_field_of_study__(
                    discipline_block_module_object,
                    gia_not_for_del,
                    isu_academic_plan_json,
                )

            AcademicPlanUpdateProcessor.__del_work_program_in_field_of_study__(
                discipline_block_module_object,
                discipline_not_for_del,
                isu_academic_plan_json,
            )
            if work_program_change_in_discipline_block_modules_not_for_del:
                AcademicPlanUpdateProcessor.__del_old_wpcbms__(
                    discipline_block_module_object,
                    work_program_change_in_discipline_block_modules_not_for_del,
                )
            if discipline_not_for_del:
                AcademicPlanUpdateProcessor.__del_old_wpcbms_by_module__(
                    discipline_block_module_object, discipline_not_for_del
                )
            ...
            if "practice" in isu_academic_plan_discipline_json["rpdUrl"]:
                ...
                "gia" in isu_academic_plan_discipline_json["rpdUrl"]
                or "Блок 3. ГИА" in isu_academic_plan_discipline_json["blockName"]
            ):
                gia_object = AcademicPlanUpdateProcessor.__process_gia__(
                    isu_academic_plan_json,
                    isu_academic_plan_discipline_json,
                    father_module_object,
                )
                print("gia", gia_object)
                (
                    gia_in_field_of_study_object,
                    work_program_change_in_discipline_block_module_object,
                ) = AcademicPlanUpdateProcessor.__process_gia_linked_data__(
                    father_module_object,
                    gia_object,
                    isu_academic_plan_discipline_json,
                    isu_academic_plan_json,
                ...
                return gia_object, work_program_change_in_discipline_block_module_object

            if isu_academic_plan_discipline_json["id"] == 29867:
                work_program_object = AcademicPlanUpdateProcessor.__process_english__(
                    isu_academic_plan_json,
                    isu_academic_plan_discipline_json,
                    father_module_object,
                )
                return work_program_object, None
        ...
        for plan_id in academic_plans_ids:
            try:
                ...
                imp = ImplementationAcademicPlan.objects.get(
                    academic_plan=old_academic_plan
                )
                ...
                if isu_academic_plan_json is not None:
                    ...
                    for block in isu_academic_plan_json["structure"]:
                        ...
                        for module in block["children"]:
                            ...
                            father_module, _ = self.recursion_module_updater(
                                module,
                                isu_academic_plan_json,
                                discipline_block_object,
                                None,
                    ...
                    academic_plan_update_configuration = (
                        AcademicPlanUpdateConfiguration.objects.get(
                            academic_plan_id=plan_id
                        )
                    )
                    academic_plan_update_configuration.updated_date_time = (
                        timezone.now()
                    )
    ...
    def __process_practice__(
        ...
        cerf_list = []
        discipline_contents = list(
            isu_academic_plan_practice_json["contents"].values()
        )[0]
            ...
            for activity in sem_dict["activities"]:
                ...
                if activity["workTypeId"] in certification_types.keys():
                    activity_list.append(
                        int(certification_types[activity["workTypeId"]])
                    )
        ...
        practice_object.structural_unit = choose_department(
            isu_academic_plan_practice_json
        )
        practice_object.ze_v_sem = process_hours(ze_v_sem)
        practice_object.number_of_semesters = isu_academic_plan_practice_json[
            "disciplineDuration"
        ]
    ...
    @staticmethod
    @AcademicPlanUpdateAspect.gia_changes_aspect
    def __process_gia__(
        gia_object, isu_academic_plan_json, isu_academic_plan_gia_json, module_object
    ):
        if gia_object is None:
            pt = GIABaseTemplate.objects.create()
            gia_object = GIA(
                title=isu_academic_plan_gia_json["name"].strip(),
                # subject_code=isu_academic_plan_gia_json['plan_order'],
                # qualification=AcademicPlanUpdateUtils.get_qualification(isu_academic_plan_json),
                # prac_isu_id=str(isu_academic_plan_gia_json['id']),
                discipline_code=str(isu_academic_plan_gia_json["id"]),
            )
            gia_object.save()
            gia_object.gia_base = pt
            gia_object.save()
        ...
        certification_types = {5: "1", 9: "2", 6: "3", 7: "4", 8: "5"}
        cerf_list = []
        discipline_contents = list(isu_academic_plan_gia_json["contents"].values())[0]
        for sem_dict in discipline_contents:
            sem = sem_dict["order"] - 1
            ze_v_sem[sem] = sem_dict["creditPoints"]
            activity_list = []
        ...
        # work_program_object.description = isu_academic_plan_discipline_json["description"]
        gia_object.language = isu_academic_plan_gia_json["langCode"].lower()
        gia_object.structural_unit = choose_department(isu_academic_plan_gia_json)
        gia_object.ze_v_sem = process_hours(ze_v_sem)
        gia_object.number_of_semesters = isu_academic_plan_gia_json[
            "disciplineDuration"
        ]
