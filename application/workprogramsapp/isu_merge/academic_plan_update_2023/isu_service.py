...
class IsuService:
    def __init__(self, isu_user):
        ...
        self.academic_plan_only_modules_url = (
            "https://dev.api.itmo.su/study_plan_admin/api/study_plan"
        )
        self.academic_plan_headers_url = (
            "https://disc.itmo.su/api/v1/academic_plans_heading"
        )
        self.modules_url = (
            "https://references.itmo.su/api/v1/references/modules?limit=50000&offset=0"
        )
        self.grant_type = "client_credentials"

    def get_access_token(self, add_headers=None):
        auth_data = {
            "client_id": self.isu_user.client_id,
            "client_secret": self.isu_user.client_secret,
    ...
    def get_academic_plan(self, academic_plan_id):
        ...
        if response.status_code != 200:
            ...
            raise Exception(
                "Error: academic plan fetch failed: id: {id}, responce_code: {code}".format(
                    code=str(response.status_code), id=str(academic_plan_id)
                )
            )
    ...
    def get_academic_plan_only_modules(self, academic_plan_id):
        self.__check_token__()
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Token " + self.token,
        }
        url = self.academic_plan_only_modules_url + "/" + academic_plan_id
        response = requests.get(url, headers=headers)
