...
from workprogramsapp.isu_merge.academic_plan_update_2023.academic_plan_update_logger import (
...
class AcademicPlanUpdateAspect:
    ...
    def discipline_block_module_changes_aspect(func):
        def wrapper(*args, **kwargs):
            """Тут реализовать логику поиска модулей и проверку на состав
            модуля может понадобится isu_academic_plan_block_module_json,
            который есть в этом декораторе добавим в модуль поле "дата
            последнего обновления" через DisciplineBlockModuleInIsu находим все
            таблицы, которые теоретически должны быть такие же, как эта далее
            сравниваем состав этих таблиц с модулями из жсона и смотрим на.

            дату, если таблицы менялись менее чем пару дней назад и состав
            розный - делаем новую, если более, изменяем состав (тут нужно
            подумать, как лучше)
            """
            (
            ...
            if new_id:
                if DisciplineBlockModule.objects.filter(
                    ...
                    old_discipline_block_module_object = (
                        DisciplineBlockModule.objects.filter(
                            name=isu_academic_plan_block_module_json["name"],
                            isu_module__new_id=isu_academic_plan_block_module_json[
                                "new_id"
                            ],
                        )[0]
                    )
                ...
                if DisciplineBlockModule.objects.filter(
                    ...
                    old_discipline_block_module_object = (
                        DisciplineBlockModule.objects.filter(
                            name=isu_academic_plan_block_module_json["name"],
                            isu_module__isu_id=isu_academic_plan_block_module_json[
                                "id"
                            ],
                            isu_module__new_id=None,
                        )[0]
