...
def discipline_block_module_object_relations_updater(discipline_block_module_object):
    ...
    for module in DisciplineBlockModuleInIsu.objects.filter(
...
def process_modules(modules: list):
    ...
    for isu_module in modules:
        module = DisciplineBlockModuleInIsu.objects.filter(
            isu_id=int(isu_module["id"])
        ).first()
        print("- Module selected", module)
        print("-- Isu Module ID", isu_module["id"])
        if DisciplineBlockModuleInIsu.objects.filter(
            isu_id=isu_module["id"], isu_father_id=isu_module["parent_id"]
        ).exists():
            ...
            if DisciplineBlockModuleInIsu.objects.filter(
                isu_id=isu_module["parent_id"]
            ).exists():
                isu_father_module = DisciplineBlockModuleInIsu.objects.filter(
                    isu_id=isu_module["parent_id"]
                ).first()
                ...
                isu_father_module = DisciplineBlockModuleInIsu.objects.create(
                    isu_id=isu_module["parent_id"], module=new_father_module
                )
