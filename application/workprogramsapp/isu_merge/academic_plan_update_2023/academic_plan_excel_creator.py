...
class AcademicPlanExcelCreator:
    ...
    def create_excel_file(self):
        ...
        for academic_plan_id in ids:
            ...
            training_period = int(float(academic_plan_json["training_period"]))
        ...
        academic_plan_df["ДИСЦИПЛИНА"] = academic_plan_df["ДИСЦИПЛИНА"].apply(
            AcademicPlanUpdateUtils.clean_text
        )

        academic_plan_df["МОДУЛЬ"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils().set_module(
        ...
        academic_plan_df["НОМЕР"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.num_to_int(
                row["НОМЕР_ПО_ПЛАНУ"], row["ДИСЦИПЛИНА"]
            ),
            axis=1,
        )

        academic_plan_df["ЭКЗ_ПО_СЕМЕСТРАМ"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.set_control(
                row["ЭКЗ"], row["СРОК_ОБУЧЕНИЯ"]
            ),
            axis=1,
        )

        academic_plan_df["ЗАЧЕТ_ПО_СЕМЕСТРАМ"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.set_control(
        ...
        academic_plan_df["ДИФ_ЗАЧЕТ_ПО_СЕМЕСТРАМ"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.set_control(
                row["ДИФ_ЗАЧЕТ"], row["СРОК_ОБУЧЕНИЯ"]
            ),
            axis=1,
        )

        academic_plan_df["КП_ПО_СЕМЕСТРАМ"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.set_control(
                row["КП"], row["СРОК_ОБУЧЕНИЯ"]
            ),
            axis=1,
        )

        academic_plan_df["СРС_СТАТУС"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.hours_status(
                row["ЗЕ_В_СЕМЕСТРАХ"],
                row["ЛЕК_В_СЕМЕСТРАХ"],
        ...
        academic_plan_df["СРС"] = academic_plan_df.apply(
            lambda row: AcademicPlanUpdateUtils.set_srs(
                row["СРС_СТАТУС"],
                row["ЗЕ_В_СЕМЕСТРАХ"],
                row["ЛЕК_В_СЕМЕСТРАХ"],
                row["ПРАК_В_СЕМЕСТРАХ"],
        ...
        academic_plan_df = academic_plan_df.drop(
            ["НОМЕР_ПО_ПЛАНУ", "СРОК_ОБУЧЕНИЯ"], axis=1
        )
