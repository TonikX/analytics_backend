...
from workprogramsapp.models import CertificationEvaluationTool, IsuObjectsSendLogger
...
def generate_contents(
    ...
    if volume:
        volume = int(volume)
    return {"volume": volume, "work_type_id": type_id}
...
def generate_response(url, headers, body, obj_name, obj_id, ap_id=None):
    ...
    try:
        IsuObjectsSendLogger.objects.create(
            ...
            obj_type=obj_name,
            generated_json=body,
            error_status=response.json()["error_code"],
...
def post_gia_to_isu(token, gia, ap_id) -> int:
    ...
    for i, el in enumerate(ze):
        order_dict = {
            "order": i + 1,
            "work_types": [
                generate_contents(type_id=5),
                generate_contents(type_id=4, volume=el * 36),
            ],
        }
...
def post_practice_to_isu(token, practice, ap_id) -> int:
    ...
    for i, el in enumerate(list_of_tools):
        if el:
            order_dict = {"order": i + 1, "work_types": []}
            for type_os in el:
                order_dict["work_types"].append(
                    generate_contents(type_id=certification_types[type_os])
                )
            order_dict["work_types"].append(
                generate_contents(type_id=4, volume=ze[i] * 36)
            )
            practice_dict["contents"].append(order_dict)
...
def post_wp_to_isu(token, wp, ap_id) -> tuple:
    ...
    for order, _ in enumerate(lecture_list):
        ...
        if lecture_list[order] != 0:
            order_dict["work_types"].append(
                generate_contents(volume=lecture_list[order], type_id=1)
            )
        if lab_list[order] != 0:
            order_dict["work_types"].append(
                generate_contents(order=order, volume=lab_list[order], type_id=2)
            )
        if practice_list[order] != 0:
            order_dict["work_types"].append(
                generate_contents(order=order, volume=practice_list[order], type_id=3)
            )
        if sro_list[order] != 0:
            fake_sro = (
                36 * ze[order]
            ...
            order_dict["work_types"].append(
                generate_contents(order=order, volume=fake_sro, type_id=4)
            )
        if cons_list[order] != 0:
            order_dict["work_types"].append(
                generate_contents(order=order, volume=cons_list[order], type_id=12)
            )
    ...
    for cerf in CertificationEvaluationTool.objects.filter(work_program=wp):
        ...
        for term_dict in wp_dict["contents"]:
            if term_dict["order"] == cerf.semester:
                founded = True
                term_dict["work_types"].append(
                    generate_contents(type_id=certification_types[cerf.type])
                )
        if not founded:
            order_dict = {
                "order": cerf.semester,
...
def post_module_to_isu(token, module, parent_id, block, ap_id):
    ...
    if module.selection_parametr:
        module_dict["params"] = [
            int(el) for el in module.selection_parametr.split(", ")
        ]
...
def generate_wp_in_lower_module_for_ap_isu(
    disc_id, changeblock, module, isu_id_lower_module, required
):
    replaceable = False
    # required = False
