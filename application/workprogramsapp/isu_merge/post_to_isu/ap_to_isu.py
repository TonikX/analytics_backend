...
from workprogramsapp.isu_merge.post_to_isu.updaters_isu_logic import (
    generate_wp_in_lower_module_for_ap_isu,
    post_ap_to_isu,
    post_gia_to_isu,
    post_module_to_isu,
...
def recursion_ap_to_isu(
    ...
    for module in modules:
        ...
        isu_module = DisciplineBlockModuleInIsu.objects.filter(
            module=module, isu_father_id=father_id, academic_plan=ap
        ).last()
        if isu_module:
            id_created = isu_module.isu_id
        else:
            id_created = post_module_to_isu(
                token=TOKEN,
                module=module,
                block=block,
                parent_id=father_id,
        ...
        if module.childs.all().exists():
            recursion_ap_to_isu(
                module.childs.all(), lines_of_plan, block, ap, id_created, required
            )
        else:
            for changeblock in WorkProgramChangeInDisciplineBlockModule.objects.filter(
                ...
                if not disc_id:
                    if program_type == "wp":
                        ...
                        disc_id = post_practice_to_isu(
                            token=TOKEN, practice=prac_obj, ap_id=ap.id
                        )
...
def ap_isu_generate_dict(ap):
    ...
    for block in DisciplineBlock.objects.filter(academic_plan=ap).order_by("name"):
        recursion_ap_to_isu(
            ...
            lines_of_plan=ap_dict["lines_of_plans"],
            block=block,
            ap=ap,
        )
    (
        ...
        code,
        json_dict,
    ) = post_ap_to_isu(TOKEN, ap_dict, ap)
