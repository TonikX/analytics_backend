...
from workprogramsapp.bars_merge.bars_api_getter import (
    get_educational_program_main,
    get_disciplines,
    get_one_educational_program,
    get_list_of_regular_checkpoints,
    post_checkpoint_plan,
)
from workprogramsapp.bars_merge.bars_function_for_threading import (
    generate_single_checkpoint,
    academicNTCheckpointGenerator,
)
from workprogramsapp.bars_merge.checkpoint_template import (
    generate_checkpoint,
    get_checkpoints_type,
    generate_discipline,
    generate_checkpoint_plan,
    generate_fos,
    generate_checkpoint_new,
)
...
from workprogramsapp.bars_merge.serializers import (
    BarsEPAssociateSerializer,
    BarsWorkProgramsAssociateSerializer,
            ...
            try:
                field_of_study = list(
                    ImplementationAcademicPlan.objects.filter(
                        field_of_study__number=program["code"][
                            : program["code"].rfind(".")
                        ]
                    )
                )
...
def FindSimilarWP(request):
    ...
    for program in wp:
        if not BarsWorkProgramsAssociate.objects.filter(bars_id=program["id"]):
            ...
            if wp_to_append:
                wp_object = BarsWorkProgramsAssociate(
                    bars_id=program["id"], term=program["term"]
                )
                wp_object.save()
...
def CreateCheckPoint(request):
    ...
    bars_id = WorkProgram.objects.get(id=work_program_id).wp_isu_id
    types_checkpoints = (
        get_list_of_regular_checkpoints()
    )  # Список оценочных средств в БАРСе
    list_regular = []
    ...
    discipline = None
    extra_points = (
        True if WorkProgram.objects.get(id=work_program_id).extra_points else False
    )
    field_of_study = FieldOfStudy.objects.get(id=field_of_study_id)

    # Получение дисцплины из направления
    wp_in_change = WorkProgramChangeInDisciplineBlockModule.objects.filter(
    ...
    evaluation_tools = (
        EvaluationTool.objects.filter(
            evaluation_tools__in=DisciplineSection.objects.filter(
                work_program__id=work_program_id
            )
        )
        .distinct()
        .filter(semester=needed_semester)
    )
    ...
    certificate = CertificationEvaluationTool.objects.filter(
        work_program=work_program_id, semester=needed_semester
    )
    for cerf in certificate:
        if int(cerf.type) == 4:
            has_course_project = True
            course_project = generate_checkpoint(
                name=cerf.name,
                min=cerf.min,
                max=cerf.max,
                week=None,
    ...
    discipline = generate_discipline(
        bars_id=bars_id,
        name=WorkProgram.objects.get(id=work_program_id).title,
        term=term,
...
def SendCheckpointsForAcceptedWP(request):
    ...
    types_checkpoints = get_list_of_regular_checkpoints(
        setup_bars
    )  # получаем список типов чекпоинтов из БАРС
    if send_semester == 0:
        send_semester = 1
    else:
        send_semester = 0
    ...
    if not one_wp:
        needed_wp = WorkProgram.objects.filter(
            expertise_with_rpd__expertise_status__contains="AC", bars=True
        ).distinct()
        just_accepted_wp = WorkProgram.objects.filter(
            accepted_wp_in_bars__year_of_study=setup_bars[0],
            accepted_wp_in_bars__semester_of_sending=setup_bars[1],
        ).distinct()
    else:
    ...
    for work_program in needed_wp:  # для каждой РПД формируем отдельный запрос в БАРС
        ...
        for now_semester in range(0, 12):  # Цикл по семестрам\
            ...
            if (
                work_program.id in wp_for_many_terms_list
                or work_program.discipline_code in isu_bank_many_term
            ):
                many_term_regex = r""
                for i in range(12):
                    if i == now_semester:
                ...
                implementation_of_academic_plan_all = ImplementationAcademicPlan.objects.filter(
                    ...
                    academic_plan__discipline_blocks_in_academic_plan__modules_in_discipline_block__change_blocks_of_work_programs_in_modules__zuns_for_cb__zuns_for_wp__ze_v_sem__iregex=many_term_regex,
                ).distinct()
                implementation_of_academic_plan = (
                    implementation_of_academic_plan_all.filter(
                ...
                imp_list_for_many_term = list(
                    {v["id"]: v for v in imp_list_for_many_term}.values()
                )
            ...
            implementation_of_academic_plan_all = ImplementationAcademicPlan.objects.filter(
                academic_plan__discipline_blocks_in_academic_plan__modules_in_discipline_block__change_blocks_of_work_programs_in_modules__work_program=work_program,
                academic_plan__discipline_blocks_in_academic_plan__modules_in_discipline_block__change_blocks_of_work_programs_in_modules__zuns_for_cb__zuns_for_wp__ze_v_sem__iregex=cred_regex,
            ).distinct()
            ...
            for count_relative in range(0, max_sem):
                ...
                for imp in implementation_of_academic_plan:
                    ...
                    try:
                        ...
                        for fos in field_of_studies:
                            imp_list.append(
                                generate_fos(imp.ns_id, fos.number, imp.title)
                            )
                        """isu_wp = \
                            list(WorkProgramIdStrUpForIsu.objects.filter(
                                work_program_in_field_of_study__work_program=work_program,
                ...
                imp_list = list(
                    {v["id"]: v for v in imp_list}.values()
                )  # Оставляем уникальные значения по айдишникам
                ...
                if (
                    imp_list
                    and isu_wp_id
                    and current_term % 2 == send_semester
                    and not (
                        work_program.id in wp_for_many_terms_list
                        or work_program.discipline_code in isu_bank_many_term
                    )
                    ...
                    request_text = generate_single_checkpoint(
                        absolute_semester=current_term + 1,
        ...
        if (
            work_program.id in wp_for_many_terms_list
            or work_program.discipline_code in isu_bank_many_term
        ) and minimal_sem_for_many_term != 0:
            ...
            request_text = generate_single_checkpoint(
                absolute_semester=absolute_semester,
            ...
            request_text["terms"] = [
                i
                for i in range(minimal_sem_for_many_term, maximal_sem_for_many_term + 1)
            ]
...
def postAcademicNTCheckpoints(request):
    ...
    needed_wp = WorkProgram.objects.filter(
        ...
        expertise_with_rpd__date_of_last_change__gte=date(
            year=int(date_split[2]), month=int(date_split[1]), day=int(date_split[0])
        ),
    ).distinct()
    all_sends = []  # Список всего того что отправили в барс, нужен для респонса
    for work_program in needed_wp:  # для каждой РПД формируем отдельный запрос в БАРС
...
def AddAcceptedWpToTableForAcceptedWp(request):
    ...
    if status_filter == "superkick":
        for wp in WorkProgram.objects.filter(
            wp_in_send_history__request_status=200
        ).distinct():
            AcceptedBarsInWp.objects.create(
                work_program=wp, year_of_study="2021/2022", semester_of_sending=1
            )
...
def SetBarsPointerTrueToWP(request):
    for semester in range(0, 4):
        ...
        wp = WorkProgram.objects.filter(
            ...
            expertise_with_rpd__expertise_status__contains="AC",
            zuns_for_wp__work_program_change_in_discipline_block_module__discipline_block_module__descipline_block__academic_plan__academic_plan_in_field_of_study__qualification="bachelor",
        ).distinct()
        wp.update(bars=True)
    return Response("Надеюсь, ничего не сломалось")
...
def GetWPForBARS(request, isu_wp_id):
    ...
    try:
        ...
        return Response(
            {"error": "Такой рабочей программы не существует", "error_code": 404},
            status=404,
        )
    ...
    wp_dict = {
        ...
        "name": wp.title,
        "additional_points": (
            False if wp.extra_points == "0" or not wp.extra_points else True
        ),
    ...
    for sem in range(wp.number_of_semesters):
        term_dict = {
            ...
            "regular_checkpoints": [],
            "final_checkpoint": None,
            "course_project_checkpoint": None,
            "point_distribution": None,
        }
        evaluation_tools = EvaluationTool.objects.filter(
            evaluation_tools__work_program__id=wp.id, semester=sem + 1
        ).distinct()

        certificate = CertificationEvaluationTool.objects.filter(
            work_program__id=wp.id, semester=sem + 1
        )
        print(evaluation_tools)
        for cerf in certificate:
            # Отдельно обрабатываем случай наличия курсовика
            if int(cerf.type) == 4 or int(cerf.type) == 5:
                course_project = generate_checkpoint_new(
                    name=cerf.name,
                    min=cerf.min,
                    max=cerf.max,
                    week=None,
                ...
                try:
                    ...
                    point_distribution = None
                final_checkpoint = generate_checkpoint_new(
                    name=cerf.name,
                    min=cerf.min,
                    max=cerf.max,
                    week=None,
                    type_name=get_checkpoints_type(int(cerf.type), get_name=True),
                    key=False,
                )
