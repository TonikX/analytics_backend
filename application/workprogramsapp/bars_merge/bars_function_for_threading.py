...
def generate_single_checkpoint(
    work_program,
    absolute_semester,
    relative_semester,
    programs,
    ...
    extra_points = (
        False
        if work_program.extra_points == "0" or not work_program.extra_points
        else True
    )  # 3 доп. балла
    ...
    evaluation_tools = (
        ...
        .distinct()
        .filter(semester=relative_semester)
    )
    ...
    for eva in evaluation_tools:
        ...
        if (
            types_checkpoints
        ):  # для ЦДО, если мы не передаем типы чекпоинтов, то не будем искать их в БАРСе
            id = None
            test_id = -1
                ...
                if el["name"] == checkpoint_name:
                    ...
                    if el["name"] == "Электронное тестирование в ЦДО":
                        ...
                        for test in test_list:
                            ...
                            if (test["name"] == eva.name) and (
                                test["term"] == absolute_semester
                            ):
                                test_id = test["id"]
                                break
                        ...
                        if test_id == -1:
                            body = generate_test(
                                term=absolute_semester, year=setup[0], name=eva.name
                            )
...
def academicNTCheckpointGenerator(
    work_program: WorkProgram, send_semester: int, setup_bars: tuple
) -> list:
    ...
    for now_semester in range(0, 12):  # Цикл по семестрам
        ...
        for imp in implementation_of_academic_plan:
            ...
            isu_wp = list(
                WorkProgramIdStrUpForIsu.objects.filter(
        ...
        if imp_list:  # Если такой существует
            ...
            # el govnokod
            date_of_acception_to_wp = str(
                Expertise.objects.get(work_program=work_program).date_of_last_change
            )
            date_of_acception_to_wp = date_of_acception_to_wp[
                : date_of_acception_to_wp.find(" ")
            ]
            ...
            additional_info_for_academicnt = {
                ...
                + year_month_day[0],
                "apprenticeship": str(
                    int(years_for_apprenticeship[0]) - now_semester // 2
                )
                + "/"
                + str(int(years_for_apprenticeship[1]) - now_semester // 2),
            }
            ...
            for request_element in request_text_list:
                if (
                    request_element["programs"]
                    == additional_info_for_academicnt["programs"]
                    and request_element["apprenticeship"]
                    == additional_info_for_academicnt["apprenticeship"]
                ):
                    request_element["checkpoints"].extend(
                        additional_info_for_academicnt["checkpoints"]
                    )
