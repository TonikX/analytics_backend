...
class BarsWorkProgramsAssociate(models.Model):
    ...
    base_work_programs = models.ManyToManyField(
        "WorkProgram",
        verbose_name="Рабочая программа",
        related_name="work_program_in_bars",
    )
    is_in_bars = models.BooleanField(
        verbose_name="Отправлялась ли в БАРС", default=False
    )
...
class HistoryOfSendingToBars(models.Model):
    ...
    request_text = models.TextField(
        verbose_name="Текст запроса в БАРС", blank=True, null=True
    )
    request_response = models.TextField(
        verbose_name="Текст ответа БАРС", blank=True, null=True
    )
    request_status = models.IntegerField(
        verbose_name="Статус запроса в БАРС", blank=True, null=True
    )
...
class AcceptedBarsInWp(models.Model):
    ...
    semester_of_sending = models.IntegerField(
        verbose_name="Весенний или осенний семестр", blank=True, null=True
    )


class BarsEPAssociate(models.Model):
    bars_id = models.PositiveIntegerField(verbose_name="Id ОП в системе БАРС 2.0")
    base_field_of_study = models.ManyToManyField(
