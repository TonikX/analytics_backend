...
def login(setup):
    url = BASE_URL + "/login"
    body = {
        "login": settings.BARS["BARS_LOGIN"],
        "password": settings.BARS["BARS_PASSWORD"],
    }
    ...
    requests.post(
        BASE_URL + "/config/personal", data=json.dumps(body_year), headers=BASE_HEADERS
    )
    body_term = {"name": "current_term", "value": setup[1]}
    requests.post(
        BASE_URL + "/config/personal", data=json.dumps(body_term), headers=BASE_HEADERS
    )
...
def get_educational_program_main():
    ...
    for discipline in disciplines:
        x = threading.Thread(
            target=get_get_educational_program_request,
