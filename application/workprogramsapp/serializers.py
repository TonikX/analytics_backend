...
from .models import WorkProgram, Indicator, Competence, OutcomesOfWorkProgram, DisciplineSection, Topic, EvaluationTool, \
    ...
    WorkProgramChangeInDisciplineBlockModule, Zun, WorkProgramInFieldOfStudy, Expertise, UserExpertise, \
    ExpertiseComments
...
class CommentSerializer(serializers.ModelSerializer):
    """
    При отправке комментария автоматически создает пользовательскую экспертизу,
    указывающуюю на рабочую программу (адресс отправки запроса) и пользователя,
    отправившего запрос
    """
    ...
    def create(self, validated_data):
        ...
        request=self.context.get('request')
        user_expertise=UserExpertise.objects.get(expertise__work_program=view.kwargs['pk'], expert=request.user)
