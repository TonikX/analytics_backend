...
class UserExpertise(models.Model):
    ...
    stuff_status = models.CharField(
        choices=STUFF_STATUS_CHOICES,
        max_length=1024,
        verbose_name="Роль эксперта",
        default="EX",
    )
    user_expertise_status = models.CharField(
        choices=USER_EXPERTISE_STATUS_CHOISES,
        max_length=1024,
        blank=True,
        null=True,
        verbose_name="статус экспертизы пользователя",
    )
    expert_result = models.CharField(
        verbose_name="Результаты экспертизы", max_length=50000, blank=True, null=True
    )
...
class Expertise(models.Model):
    ...
    work_program = models.ForeignKey(
        "WorkProgram",
        related_name="expertise_with_rpd",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    gia = models.ForeignKey(
        ...
        related_name="expertise_with_gia",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    practice = models.ForeignKey(
        ...
        related_name="expertise_with_practice",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    ...
    expertise_status = models.CharField(
    ...
    date_of_last_change = models.DateTimeField(
        editable=True, auto_now=True, blank=True, null=True
    )
    expertise_counter = models.IntegerField(
        blank=True, null=True, verbose_name="Счетчик отправки на экспертизу", default=0
    )
...
class ExpertiseComments(models.Model):
    ...
    comment_block = models.CharField(
        choices=BLOCK_CHOICES, max_length=1024, verbose_name="Блок комментария"
    )
    user_expertise = models.ForeignKey(
        "UserExpertise", on_delete=models.CASCADE, related_name="user_expertise_comment"
    )
...
class ExpertsOnStructuralUnit(models.Model):
    expert = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="experts_with_units",
        on_delete=models.CASCADE,
    )
    structural_units = models.ManyToManyField(
        "StructuralUnit",
        verbose_name="Эксперты",
...
class ExpertiseChangeLog(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="user_expertise_log",
        blank=True,
        null=True,
    )
    expertise = models.ForeignKey(
        "Expertise",
        on_delete=models.CASCADE,
        related_name="expertise_status_log",
        blank=True,
        null=True,
    )
