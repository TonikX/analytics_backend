...
def create_profile(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if expertise.expertise_type == "WP" or not expertise.expertise_type:
            ...
            wp_exp = WorkProgram.objects.get(
                expertise_with_rpd__expertse_users_in_rpd=instance
            )
            ...
            wp_exp = Practice.objects.get(
                expertise_with_practice__expertse_users_in_rpd=instance
            )
        ExpertiseNotification.objects.create(
...
def expertise_notificator(sender, instance, created, **kwargs):
    ...
    if instance.expertise_type == "WP" or not instance.expertise_type:
        ...
        struct_users = User.objects.filter(
            user_for_structural_unit__status__in=["leader", "deputy"],
            user_for_structural_unit__structural_unit__workprogram_in_structural_unit__expertise_with_rpd=instance,
        ).distinct()
        users = User.objects.filter(
            expertse_in_rpd__expertise__work_program=wp_exp
        ).distinct()
        ...
        struct_users = User.objects.filter(
            user_for_structural_unit__status__in=["leader", "deputy"],
            user_for_structural_unit__structural_unit__gia_in_structural_unit__expertise_with_gia=instance,
        ).distinct()
        ...
        struct_users = User.objects.filter(
            user_for_structural_unit__status__in=["leader", "deputy"],
            user_for_structural_unit__structural_unit__practice_in_structural_unit__expertise_with_practice=instance,
        ).distinct()
        users = User.objects.filter(
            expertse_in_rpd__expertise__practice=wp_exp
        ).distinct()
    else:
        struct_users = []
    ...
    if instance.expertise_status == "RE" or instance.expertise_status == "AC":
        ...
        user_to_send = users.filter(
            expertise_status_notification=True, do_email_notifications=True
        )
        user_email = [user.email for user in user_to_send]
        mail_sender(
            topic=f'Экспертиза для {name_of_object} "{wp_exp.title}" поменяла свой статус.',
        ...
        if not settings.DEBUG:
            isu_client_credentials_request(
                "https://disc.itmo.su/api/v1/disciplines/{disc_id}?status={status}&url=https://op.itmo.ru/work-program/{wp_id}".format(
                    ...
                    status=instance.get_expertise_status_display(),
                    wp_id=wp_exp.id,
                )
            )
    except:
...
def comment_notificator(sender, instance, created, **kwargs):
    wp_exp = WorkProgram.objects.get(
        expertise_with_rpd__expertse_users_in_rpd__user_expertise_comment=instance
    )
    user_sender = User.objects.get(expertse_in_rpd__user_expertise_comment=instance)

    user_to_send = User.objects.filter(
        editors=wp_exp,
        expertise_comments_notification=True,
        do_email_notifications=True,
    )
    user_to_send = user_to_send.exclude(id=user_sender.id)
    user_email = [user.email for user in user_to_send]
    mail_sender(
        topic=f'В экспертизе для рабочей программы "{wp_exp.title}" был оставлен новый комментарий',
    ...
    for editor in User.objects.filter(editors=wp_exp):
        if editor != user_sender:
            NotificationComments.objects.create(
                comment_new=instance,
                user=editor,
    ...
    for user in User.objects.filter(
        expertse_in_rpd__expertise__work_program=wp_exp,
        expertse_in_rpd__stuff_status="EX",
        expertse_in_rpd__user_expertise_comment__comment_block=instance.comment_block,
    ).distinct():
        if user != user_sender:
            NotificationComments.objects.create(
                comment_new=instance,
    ...
    read_notifications_array = [
        ...
        for x in wp_exp.read_notifications.split(", ")
    ]
...
def zun_saver(sender, instance, using, **kwargs):
    wp_in_fs_id_strs = WorkProgramIdStrUpForIsu.objects.filter(
        work_program_in_field_of_study=instance.id
    )
...
def check_previous_mode(sender, instance, *args, **kwargs):
    ...
    if instance.on_check != original_status and original_status in [
        "in_work",
        "on_check",
    ]:
        ...
        if instance.on_check == "in_work":
            ...
            status = "Одобрено"
        imp = ImplementationAcademicPlan.objects.filter(academic_plan__id=instance.id)[
            0
        ]
        users = User.objects.filter(
            implementation_academic_plan_block_modules__academic_plan__id=instance.id
        ).distinct()
        for user in users:
            AcademicPlanUpdateNotification.objects.create(
                academic_plan=instance,
                user=user,
        ...
        user_email = [user.email for user in user_to_send]
        mail_sender(
            topic=f'Учебный план "{imp.title}" поменял свой статус.',
...
# Перерасчет трудоемкости при изменении модуля
...
def count_module_ze_deleted_if_changed(
    sender, instance: DisciplineBlockModule, *args, **kwargs
):
    rewrite_ze_up(instance)
    print("d4 bad")
...
def count_wpcb_gia_ze_if_changed(
    sender, instance: WorkProgramChangeInDisciplineBlockModule, *args, **kwargs
):
...
def count_wpcb_ze_practice_if_changed(
    sender, instance: WorkProgramChangeInDisciplineBlockModule, *args, **kwargs
):
...
def count_wpcb_ze_practice_if_deleted(
    sender, instance: WorkProgramChangeInDisciplineBlockModule, *args, **kwargs
):
    rewrite_ze_up(instance.discipline_block_module)
    print("d4 bad")
