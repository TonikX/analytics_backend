...
from workprogramsapp.ap_improvement.module_ze_counter import (
    make_modules_cte_down_for_matrix,
)
...
from workprogramsapp.models import GeneralCharacteristics, DisciplineBlockModule
...
def GetCompetenceMatrixCTE(request, gen_pk):
    ...
    pk_competences = PkCompetencesInGroupOfGeneralCharacteristicSerializerForMatrix(
        ...
        .prefetch_related(
            ...
            "indicator_of_competence_in_group_of_pk_competences__indicator",
        )
    ...
    key_competences = KeyCompetencesInGroupOfGeneralCharacteristicSerializer(
        ...
        .prefetch_related(
            ...
            "indicator_of_competence_in_group_of_key_competences__indicator",
        )
        .order_by("competence__number")
        .distinct(),
        many=True,
    ...
    over_prof_competences = OverProfCompetencesInGroupOfGeneralCharacteristicSerializer(
        ...
        .prefetch_related(
            ...
            "indicator_of_competence_in_group_of_over_prof_competences__indicator",
        )
        .order_by("competence__number")
        .distinct(),
        many=True,
    ...
    for ap in academic_plans:
        ...
        for block in academic_plan.discipline_blocks_in_academic_plan.all():
            ...
            modules_lower = DisciplineBlockModule.objects.filter(
                id__in=modules_lower_ids
            ).prefetch_related(
            ...
            for module in modules_lower:
                ...
                for (
                    change_block
                ) in module.change_blocks_of_work_programs_in_modules.all():
                    change_block_dict = {
                            ...
                            change_block_dict["work_program"].append(
                                {
                                    ...
                                    "title": wp_in_fs.work_program.title,
                    ...
                    for practice_in_fs in change_block.zuns_for_cb_for_practice.all():
                        if (
                            practice_in_fs.practice.id not in unique_practice
                        ) or first_ap_iter:
                            change_block_dict["practice"].append(
                                {
                                    "id": practice_in_fs.practice.id,
                                    "title": practice_in_fs.practice.title,
                                    "competences": get_competences_practice(
                                        practice_in_fs
                                    ),
                                }
                            )
    ...
    competence_matrix["educational_program"] = (
        ImplementationAcademicPlanForCompsSSerializer(academic_plans, many=True).data
    )
