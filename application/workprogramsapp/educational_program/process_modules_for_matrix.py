...
from gia_practice_app.Practice.serializers import PracticeInFieldOfStudyCreateSerializer
...
def get_competences_practice(practice_in_fs):
    competences = Competence.objects.filter(
        indicator_in_competencse__zun_practice__practice_in_fs=practice_in_fs
    ).distinct()
    competences_dict = []
    for competence in competences:
        zuns = ZunPractice.objects.filter(
            practice_in_fs=practice_in_fs,
            ...
            zuns_array.append(
                {
                    ...
                    "wp_in_fs": PracticeInFieldOfStudyCreateSerializer(
                        PracticeInFieldOfStudy.objects.get(zun_in_practice=zun.id)
                    ).data["id"],
                }
            )
...
def get_competences_wp(wp_in_fs):
    competences = Competence.objects.filter(
        indicator_in_competencse__zun__wp_in_fs=wp_in_fs
    ).distinct()
    competences_dict = []
    for competence in competences:
        zuns = Zun.objects.filter(
            wp_in_fs=wp_in_fs, indicator_in_zun__competence__id=competence.id
        )
        zuns_array = []
        for zun in zuns:
            try:
...
def recursion_module_matrix(
    block_module, unique_wp, unique_gia, unique_practice, first_ap_iter
):
    block_module_dict = {
        "name": block_module.name,
        "type": block_module.type,
        "change_blocks_of_work_programs_in_modules": [],
        "childs": [],
    }
    # block_dict["modules_in_discipline_block"].append(block_module_dict)
    childs = block_module.childs.prefetch_related(
        "change_blocks_of_work_programs_in_modules", "childs"
    )
    if childs.exists():
        for child in childs:
            lower_module = recursion_module_matrix(
                child, unique_wp, unique_gia, unique_practice, first_ap_iter
            )
            if (
                lower_module["change_blocks_of_work_programs_in_modules"]
                or lower_module["childs"]
            ):
                block_module_dict["childs"].append(lower_module)
    for (
        change_block
    ) in block_module.change_blocks_of_work_programs_in_modules.prefetch_related(
        "work_program", "practice", "gia"
    ):
        change_block_dict = {
            "change_type": change_block.change_type,
            "credit_units": change_block.credit_units,
            "work_program": [],
            "practice": [],
            "gia": [],
        }
        ...
        if (
            change_block_dict["work_program"]
            or change_block_dict["practice"]
            or change_block_dict["gia"]
        ):
            block_module_dict["change_blocks_of_work_programs_in_modules"].append(
                change_block_dict
            )
