...
class GroupOfPkCompetencesInGeneralCharacteristic(models.Model):
    ...
    type_of_pk_competence = models.CharField(
        choices=types,
        max_length=15,
...
class PkCompetencesInGroupOfGeneralCharacteristic(models.Model):
    ...
    generalized_labor_functions = models.ManyToManyField(
        "GeneralizedLaborFunctions",
        verbose_name="Выбранные обобщенные трудовые функции",
        blank=True,
    )
    kinds_of_activity = models.ForeignKey(
        "KindsOfActivity",
        on_delete=models.SET_NULL,
        verbose_name="Сферы проф. деятельности",
        blank=True,
        null=True,
    )
    kinds_of_activity_for_miner = models.CharField(
        max_length=2048, verbose_name="трудовая функция", blank=True, null=True
    )
...
class IndicatorInPkCompetenceInGeneralCharacteristic(models.Model):
    ...
    competence_in_group_of_pk = models.ForeignKey(
    ...
    indicator = models.ForeignKey(
        "Indicator",
        on_delete=models.CASCADE,
        verbose_name="Индикатор ПК компетенции в ОХ",
        related_name="indicator_in_pk",
    )
