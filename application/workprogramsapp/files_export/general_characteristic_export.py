...
from workprogramsapp.models import FieldOfStudy, EmployerRepresentative
...
def generate_context(gh):
    ...
    if gh.is_global_educational_program:
        context[
            "realization_form"
        ] += "имеет статус международной образовательной программы (МОП), "
    if gh.is_online_format:
        context["realization_form"] += "в сетевой форме "
    if gh.collaboration_russian_in_online_format:
        context[
            "realization_form"
        ] += f"совместно с российским(-и) партнером(-ами): {gh.collaboration_russian_in_online_format}, "
    ...
    if gh.collaboration_foreign:
        context[
            ...
            f"совместно с иностранным(-и) партнером(-ами): {gh.collaboration_foreign}, "
        )
    ...
    for i, area in enumerate(gh.area_of_activity.all()):
        context["area_of_activity"].append(
            {
                "number": i + 1,
                "codename": f"{area.code_of_prof_area} {area.name_of_prof_area}",
                "code": area.code,
                "title": area.title,
            }
        )
    ...
    for group in GroupOfKeyCompetencesInEducationalStandard.objects.filter(
        ...
        for (
            competence_in_group
        ) in KeyCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk=group
        ):
            competence_dict = {
            ...
            for (
                indicator_in_group
            ) in IndicatorInKeyCompetenceInGeneralCharacteristic.objects.filter(
    ...
    for group in GroupOfGeneralProfCompetencesInEducationalStandard.objects.filter(
        ...
        for (
            competence_in_group
        ) in GeneralProfCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk=group
        ):
            competence_dict = {
                "codename": f"{competence_in_group.competence.number} {competence_in_group.competence.name}",
            ...
            for (
                indicator_in_group
            ) in IndicatorInGeneralProfCompetenceInGeneralCharacteristic.objects.filter(
                competence_in_group_of_pk=competence_in_group
            ):
                competence_dict["indicators"].append(
                    f"{indicator_in_group.indicator.number} {indicator_in_group.indicator.name}"
    ...
    for group in GroupOfOverProfCompetencesInEducationalStandard.objects.filter(
        ...
        for (
            competence_in_group
        ) in OverProfCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk=group
        ):
            competence_dict = {
                "codename": f"{competence_in_group.competence.number} {competence_in_group.competence.name}",
            ...
            for (
                indicator_in_group
            ) in IndicatorInOverProfCompetenceInGeneralCharacteristic.objects.filter(
                competence_in_group_of_pk=competence_in_group
            ):
                competence_dict["indicators"].append(
                    f"{indicator_in_group.indicator.number} {indicator_in_group.indicator.name}"
    ...
    for group in GroupOfPkCompetencesInGeneralCharacteristic.objects.filter(
        ...
        for (
            competence_in_group
        ) in PkCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk=group
        ).order_by(
            "competence__number"
        ):
            ...
            for (
                indicator_in_group
            ) in IndicatorInPkCompetenceInGeneralCharacteristic.objects.filter(
                competence_in_group_of_pk=competence_in_group
            ):
    ...
    try:
        ...
        context["ep_supervisor"] = (
            f"{gh.ep_supervisor.last_name} {gh.ep_supervisor.first_name}  "
            f"{patronymic}"
        )
    ...
    try:
        ...
        patronymic = (
            gh.dean_of_the_faculty.patronymic
            if gh.dean_of_the_faculty.patronymic
            else ""
        )
        context["dean_position"] = gh.dean_of_the_faculty_directors_position
        context["dean"] = (
            f"{gh.dean_of_the_faculty.last_name} {gh.dean_of_the_faculty.first_name} "
            f"{patronymic}"
        )
    except AttributeError:
        pass
    ...
    for emp in EmployerRepresentative.objects.filter(general_characteristic=gh):
        context["employers"].append(
            {
                "pos": emp.employer_position,
                "name": emp.fio_employer,
                "comp": emp.organization_name,
            }
        )
