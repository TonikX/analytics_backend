...
def plans_processor(file):
    ...
    for i in range(len(plans)):
        ...
        if plan_ids[0] == "nan":
            break
        create_plans_in_row(
            qualification, year, plan_ids, direction_codes, directions_names, op_name
        )

    return i


def create_plans_in_row(
    qualification: int,
    year: int,
    plan_ids: list,
    direction_codes: list,
    directions_names: list,
    op_name: str,
):
