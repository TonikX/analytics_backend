...
from workprogramsapp.files_export.academic_plan_export import process_excel
from workprogramsapp.files_export.competence_matrix_export import (
    process_excel_competence_matrix,
)
from workprogramsapp.files_export.general_characteristic_export import generate_context
from workprogramsapp.files_export.plan_logic import plans_processor
...
def render_context(context, **kwargs):
            ...
            try:
                ...
                if is_included:
                    ...
                    if wpcb["credit_units"] != None:
                        ...
                        hours_getter = hours_v2_generator(
                            context["lecture_hours_v2"],
                            context["practice_hours_v2"],
                            context["srs_hours_v2"],
                            context["lab_hours_v2"],
                        )
                        for cu in credit_units_list:
                            try:
                                if int(float(cu)) != 0:
                                    ...
                                    contact_hours = round(
                                        (hours_list[0] + hours_list[1] + hours_list[3])
                                        * 1.1,
                                        2,
                                    )
                                    semester.append(
                                        {
                                            "s": credit_units_list.index(cu) + 1,
                                            "c": cu,
                                            "h": int(cu) * 36,
                                            "lc_h": hours_list[0],
                                            "pc_h": hours_list[1],
    ...
    try:
        ...
        # По идее надо что-то сделать с циклом по блок-модулям
    ...
    wp_in_fs = WorkProgramInFieldOfStudy.objects.get(
        work_program_change_in_discipline_block_module__id=wpcb_pk,
    ...
    for z in zun_obj:
        if z.indicator_in_zun:
            tbl_competence.append(
                {
                    "competence": str(z.indicator_in_zun.competence.number)
                    + " "
                    + str(z.indicator_in_zun.competence.name),
                    "indicator": str(z.indicator_in_zun.number)
                    + " "
                    + str(z.indicator_in_zun.name),
                    "knowledge": z.knowledge,
                    "skills": z.skills,
                    "attainments": z.attainments,
                }
            )
    contact_work, lecture_classes, laboratory, practical_lessons, SRO, total_hours = (
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    for i in context["discipline_sections"]:
        ...
        if i["lecture_classes"] is None:
            ...
            lecture_classes += (
                float(i["lecture_classes"]) / context["number_of_semesters"]
            )
        ...
        if i["practical_lessons"] is None:
            ...
            practical_lessons += (
                float(i["practical_lessons"]) / context["number_of_semesters"]
            )
        ...
        total_hours += (
            0.0
            if i["total_hours"] is None
            else float(i["total_hours"]) / context["number_of_semesters"]
        )
        ...
        for j in i["topics"]:
            ...
            if j["url_online_course"] is None:
                ...
                online_names.append(
                    "{} (url: {})".format(
                        j["url_online_course"]["title"],
                        j["url_online_course"]["external_url"],
                    )
                )
    ...
    template_context["academic_plan"] = (
        str(
            ImplementationAcademicPlan.objects.filter(academic_plan__id=ap_obj.id)[
                0
            ].title
        )
        + " ("
        + str(
            FieldOfStudy.objects.filter(
                implementation_academic_plan_in_field_of_study__academic_plan__id=ap_obj.id
            )[0].number
        )
        + ")"
    )
    template_context["semester"] = semester
    template_context["total_hours_1"] = [
        round(contact_work, 2),
        round(lecture_classes, 2),
        round(laboratory, 2),
        round(practical_lessons, 2),
        round(SRO, 2),
    ]
    ...
    for bib in template_context["bibliographic_reference"]:
        if bib["description"]:
            bib["description"] = re.sub(
                "<[^>]*>", "", str(bib["description"]).replace("<", "")
            )
        else:
            bib["description"] = re.sub(
                "<[^>]*>", "", str(bib["bib_reference"]).replace("<", "")
            )
    ...
    filename = (
        str(fs_obj.number)
        + "_"
        + str(context["discipline_code"])
        + "_"
        + str(context["qualification"])
        + "_"
        + str(kwargs["year"])
        + "_"
    ...
    for i in template_context["evaluation_tools"]:
        ...
        rt.add(
            ...
            url_id=tpl.build_url_id(
                "https://op.itmo.ru/work-program/{}/evaluation-tools/{}".format(
                    context["id"], i["id"]
                )
            ),
        )
    ...
    for item in context["certification_evaluation_tools"]:
        try:
            item["url"] = (
                "https://op.itmo.ru/work-program/{}/intermediate-certification/{}".format(
                    context["id"], item["id"]
                )
            )
            rt = RichText()
    ...
    template_context["certification_evaluation_tools_semestr_2"] = (
        certification_evaluation_tools_semestr_2
    )
    template_context["certification_evaluation_tools_semestr_3"] = (
        certification_evaluation_tools_semestr_3
    )
    template_context["certification_evaluation_tools_semestr_4"] = (
        certification_evaluation_tools_semestr_4
    )
    try:
        template_context["outcomes_max_all_semester_1"] = sum(
            items_max_semester_1
        ) + int(context["extra_points"] or 0)
        template_context["outcomes_max_all_semester_2"] = sum(
            items_max_semester_2
        ) + int(context["extra_points"] or 0)
        template_context["outcomes_max_all_semester_3"] = sum(
            items_max_semester_3
        ) + int(context["extra_points"] or 0)
...
class DocxFileExportView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context, filename = render_context(
            data,
...
def render_context_syllabus(context, **kwargs):
    ...
    try:
        for wpcb in context["work_program_in_change_block"]:
            if (
                wpcb["discipline_block_module"]["descipline_block"]["academic_plan"][
                    "educational_profile"
                ]
                == ap_obj.educational_profile
            ):
                semester = [
                    (i, wpcb["credit_units"][i], wpcb["change_type"])
                    for i in range(len(wpcb["credit_units"]))
    ...
    template_context["prerequisites"] = ", ".join(
        map(str, [i["item"]["name"] for i in context["prerequisites"]])
    )
    template_context["outcomes"] = ", ".join(
        map(str, [i["item"]["name"] for i in context["outcomes"]])
    )
...
class SyllabusExportView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context, filename = render_context_syllabus(
            data,
            field_of_study_id=kwargs["fs_id"],
            academic_plan_id=kwargs["ap_id"],
            year=kwargs["year"],
...
class AcademicPlanGenerateXlsx(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        imp = ImplementationAcademicPlan.objects.filter(
        ...
        response = HttpResponse(
            content=save_virtual_workbook(wb_obj),
...
class CompetenceMatrixGenerateExcel(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        filename = (
            gen_characteristic.educational_program.all()[0].title
            + " "
            + str(gen_characteristic.educational_program.all()[0].year)
        )
