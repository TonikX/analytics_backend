...
def process_changeblock(ws, level, block_module, unique_dict, competence_dict):
    for change_block in WorkProgramChangeInDisciplineBlockModule.objects.filter(
        ...
        for work_program in WorkProgram.objects.filter(
            work_program_in_change_block=change_block
        ):
            if (work_program.id not in unique_dict["unique_wp"]) or (
                ...
                insert_cell_data(
                    ...
                    level=level,
                    column=1,
                    data=work_program.title,
                    horizontal="left",
                )
                for plus_column in find_competence_row(
                    competence_dict, competences_in_program
                ):
                    insert_cell_data(
                        ws=ws,
                        level=level,
        ...
        for practice in Practice.objects.filter(practice_in_change_block=change_block):
            if (practice.id not in unique_dict["unique_practice"]) or (
                ...
                practice_in_fs = PracticeInFieldOfStudy.objects.get(
                    practice=practice,
                    work_program_change_in_discipline_block_module=change_block,
                )
                competences_in_program = {
                ...
                fill_row(ws, level, "FFFFFF")
                insert_cell_data(
                    ws=ws, level=level, column=1, data=practice.title, horizontal="left"
                )
                for plus_column in find_competence_row(
                    competence_dict, competences_in_program
                ):
        ...
        for gia in GIA.objects.filter(gia_in_change_block=change_block):
            if (gia.id not in unique_dict["unique_gia"]) or (
                ...
                fill_row(ws, level, "FFFFFF")
                insert_cell_data(
                    ws=ws, level=level, column=1, data=gia.title, horizontal="left"
                )
                level += 1
            else:
                pass
...
def recursion_module_matrix(ws, level, modules, unique_dict, competence_dict, depth):
    for block_module in modules:
        ...
        insert_cell_data(
            ws=ws, level=level, column=1, data=block_module.name, horizontal="left"
        )
        level += 1

        childs = block_module.childs.all()
        if childs.exists():
            level = recursion_module_matrix(
                ws, level, childs, unique_dict, competence_dict, depth + 1
            )
        else:
            level = process_changeblock(
                ws, level, block_module, unique_dict, competence_dict
            )
...
def process_excel_competence_matrix(gen_characteristic):
    ...
    for ap in academic_plans:
        ...
        for block in DisciplineBlock.objects.filter(
            ...
            insert_cell_data(
                ws=ws, level=level, column=1, data=block.name, horizontal="left"
            )
            ...
            level = recursion_module_matrix(
                ws, level, modules, unique_dict, competence_dict, depth + 1
            )
...
def fill_competences(
    ws, column_start, competence_queryset, competence_dict, name_comp, string_comp
):
    level = 2
    ...
    if not name_comp == "pk_competences":
        ...
        for pk_type in ["prof", "fore", "min"]:
            for comp in competence_queryset.filter(
                group_of_pk__type_of_pk_competence=pk_type
    ...
    ws.merge_cells(
        start_row=1, end_row=1, start_column=column_start, end_column=column_end
    )
...
def competence_header(ws, gen_characteristic):
    ...
    pk_competences = (
        PkCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            ...
            group_of_pk__type_of_pk_competence="prof",
        )
        .order_by("competence__number")
        .distinct()
        | PkCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk__general_characteristic=gen_characteristic,
            competence__isnull=False,
            group_of_pk__type_of_pk_competence="fore",
        )
        .order_by("competence__number")
        .distinct()
        | PkCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk__general_characteristic=gen_characteristic,
            competence__isnull=False,
            group_of_pk__type_of_pk_competence="min",
        )
        .order_by("competence__number")
        .distinct()
    )
    general_prof_competences = (
        GeneralProfCompetencesInGroupOfGeneralCharacteristic.objects.filter(
    ...
    over_prof_competences = (
        OverProfCompetencesInGroupOfGeneralCharacteristic.objects.filter(
            group_of_pk__educational_standard=gen_characteristic.educational_standard,
            competence__isnull=False,
        ).distinct()
    )
    competence_dict = {
        "key_competences": {
            "start": 0,
            "list_num": [],
            "queryset": key_competences,
            "label": "Ключевые компетенции",
        },
        "over_prof_competences": {
            "start": 0,
            "list_num": [],
            "queryset": over_prof_competences,
            "label": "Надпрофессиональные компетенции",
        },
        "general_prof_competences": {
            "start": 0,
            "list_num": [],
            "queryset": general_prof_competences,
            "label": "Общепрофессиональные компетенции ",
        },
        "pk_competences": {
            "start": 0,
            "list_num": [],
            "queryset": pk_competences,
            "label": "Профессиональные компетенции",
        },
    }
    column_start = 2
    for key in competence_dict:
        column_start = fill_competences(
            ws=ws,
