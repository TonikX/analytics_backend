...
class IsExpertiseMaster(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        return UserStructuralUnit.objects.filter(
        ...
        if "pk" in dict(view.kwargs):
            return (
                UserExpertise.objects.filter(
                    expert=request.user, expertise=view.kwargs["pk"]
                )
                or UserExpertise.objects.filter(
                    expertise__work_program__structural_unit__user_in_structural_unit__user=request.user,
                    expertise__work_program__structural_unit__user_in_structural_unit__status__in=[
                        "leader",
                        "deputy",
                    ],
                ...
                or UserExpertise.objects.filter(
                    ...
                    expertise__practice__structural_unit__user_in_structural_unit__status__in=[
                        "leader",
                        "deputy",
                    ],
                    expertise=view.kwargs["pk"],
                ...
                or UserExpertise.objects.filter(
                    ...
                    expertise__gia__structural_unit__user_in_structural_unit__status__in=[
                        "leader",
                        "deputy",
                    ],
                    expertise=view.kwargs["pk"],
...
class IsWorkProgramMemberOfExpertise(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if "pk" in dict(view.kwargs):
            return UserExpertise.objects.filter(
                expert=request.user, expertise__work_program=view.kwargs["pk"]
            )
        else:
...
class IsMemberOfUserExpertise(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if "pk" in dict(view.kwargs):
            return (
                UserExpertise.objects.filter(expert=request.user, pk=view.kwargs["pk"])
                or UserExpertise.objects.filter(
                    expertise__work_program__structural_unit__user_in_structural_unit__user=request.user,
                    expertise__work_program__structural_unit__user_in_structural_unit__status__in=[
                        "leader",
    ...
    def has_permission(self, request, view):
        if "pk" in dict(view.kwargs):
            return WorkProgramInFolder.objects.filter(
                pk=view.kwargs["pk"], folder__owner=request.user
            )
        try:
...
class IsOwnerOfFolderWithIndividualImplementationAcademicPlan(
...
class IsDisciplineBlockModuleEditor(permissions.BasePermission):
    ...
    def check_access(module_id: int, user: User) -> bool:
        return (
            DisciplineBlockModule.objects.filter(pk=module_id, editors=user).exists()
            or user.is_expertise_master
        )
...
class IsBlockModuleEditor(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in (
            ...
            "POST",
            "PATCH",
        ) and request.user.groups.filter(name="blockmodule_editor"):
            return True
        return request.user in obj.editors.all()
...
class IsUniversalModule(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if (
            ...
            and view.__class__.__name__ == "DisciplineBlockModuleDestroyView"
        ):
            module = DisciplineBlockModule.objects.get(pk=view.kwargs["pk"])
            if (
                module.type == "universal_module"
                and request.user not in module.editors.all()
            ):
                return False
        ...
        if (
            request.method == "DELETE"
            and view.__class__.__name__ == "WorkWithBlocksApiView"
        ):
            module_id = request.query_params.get("module")
            module = DisciplineBlockModule.objects.get(id=module_id)
            if module.type == "universal_module" and not request.user.groups.filter(
        ...
        if (
            ...
            for module_id in request.data.get("module"):
                ...
                if (
                    module.type == "universal_module"
                    and not request.user.groups.filter(name="expertise_master")
                ):
                    return False
