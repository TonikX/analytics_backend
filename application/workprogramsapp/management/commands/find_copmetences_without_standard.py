...
from workprogramsapp.models import Competence, Zun
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        zuns_without_standard = Zun.objects.filter(
            indicator_in_zun__competence__in=competences_to_delete
        )
        ...
        competences_to_save = Competence.objects.filter(
            key_filter | over_filter | general_filter
        ).exclude(number__icontains="ПК")
        zuns_with_standard = Zun.objects.filter(
            indicator_in_zun__competence__in=competences_to_save
        )
        ...
        print(
            "Их процентное соотношение (не/в):",
            (zuns_without_standard.count() / zuns_with_standard.count()) * 100,
        )
        print("done")
