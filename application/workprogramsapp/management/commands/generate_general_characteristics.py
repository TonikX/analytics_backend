    ...
    def handle(self, *args, **options):
        ...
        for iap in ImplementationAcademicPlan.objects.filter(year=year):
            if GeneralCharacteristics.objects.filter(
                ...
                gh = GeneralCharacteristics.objects.filter(
                    educational_program__title=iap.title,
                    educational_program__year=iap.year,
                )[0]
                ...
                if iap.qualification == "bachelor":
                    gh.educational_standard = EducationalStandard.objects.filter(
                        name="ОС Университета ИТМО уровня бакалавриат (22)",
                        standard_date=2022,
                    )[0]
                elif iap.qualification == "master":
                    gh.educational_standard = EducationalStandard.objects.filter(
                        name="ОС Университета ИТМО уровня магистратура (22)",
                        standard_date=2022,
                    )[0]
            gh.save()
        print("done")
