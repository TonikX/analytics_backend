...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        gh_dict = {"ID_ИНДИКАТОРА": [], "КОД_ИНДИКАТОРА": [], "ИМЯ_ИНДИКАТОРА": [], "ID_ОХ": [], "ID_ПЛАНА": [],
                   "ISU_ID_ПЛАНА": [], "ИМЯ_ПЛАНА": [], "ZUN_ID": [], "ЗНАНИЯ": [], "УМЕНИЯ": [], "НАВЫКИ": []}\
        ...
        for zun_id in data_for_iter:
            ...
            try:
                ...
                continue
            gh_list = GeneralCharacteristics.objects.filter(educational_standard__standard_date=2022)
            ...
            print(gh_list)
        print(len(gh_dict["ID_ИНДИКАТОРА"]), len(gh_dict["КОД_ИНДИКАТОРА"]), len(gh_dict["ИМЯ_ИНДИКАТОРА"]),
              len(gh_dict["ID_ОХ"])
              , len(gh_dict["ID_ПЛАНА"]), len(gh_dict["ISU_ID_ПЛАНА"]), len(gh_dict["ИМЯ_ПЛАНА"]))
