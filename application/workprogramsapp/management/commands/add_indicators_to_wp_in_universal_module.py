...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        modules_for_filtering = DisciplineBlockModule.objects.annotate(
            search=SearchVector("name")
        ).filter(search="универсальная подготовка")
        ...
        for module in modules:
            changeblocks = DisciplineBlockModule.get_all_changeblocks_from_module(
                module
            )
                ...
                for wp in changeblock.work_program.all():
                    ...
                    if max_len_zuns != 0:
                        ...
                        for module_fs in module_for_fs:
                            imps = ImplementationAcademicPlan.get_all_imp_by_modules(
                                [module_fs]
                            )
                            if imps.filter(year=2023).exists():
