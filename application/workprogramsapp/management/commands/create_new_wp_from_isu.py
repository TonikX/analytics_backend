...
from workprogramsapp.models import WorkProgram, CertificationEvaluationTool
        ...
        for wp_dict in list_of_wp:
            if wp_dict["disc_type"] == 1:  # Если тип "Дисциплина"
                try:
                    ...
                    try:
                        discipline_hours = isu_logger.get_wp_hours(str(wp_dict["id"]))[
                            0
                        ]
                    except TypeError:
                        continue
                    ...
                    for sem_dict in discipline_hours["discipline_content"]:
                        ...
                        for type_dict in sem_dict["discipline_acts"]:
                            ...
                            if wt_id == 1:
                                ...
                                cerf = CertificationEvaluationTool.objects.create(
                    ...
                    practice = (
                        ", ".join([str(hour) for hour in practice_hours])
                        if practice_hours
                        else "0.0, 0, 0, 0"
                    )
                    lecture = (
                        ", ".join([str(hour) for hour in lecture_hours])
                        if lecture_hours
                        else "0.0, 0, 0, 0"
                    )
                    lab = (
                        ", ".join([str(hour) for hour in lab_hours])
                        if lab_hours
                        else "0.0, 0, 0, 0"
                    )
                    cons = (
                    ...
                    created_wp = WorkProgram.objects.create(
                        discipline_code=str(wp_dict["id"]),
