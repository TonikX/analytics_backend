...
class PracticeSerializer(serializers.ModelSerializer):
    ...
    def get_permissions_info(self, instance) -> dict[str, bool | None]:
        ...
        try:
            ...
            user_exp_queryset = UserExpertise.objects.filter(
                expert=request.user, expertise=exp
            )
    ...
    def to_representation(self, value) -> OrderedDict:
        ...
        self.fields["prerequisites"] = PrerequisitesOfPracticeSerializer(
            source="prerequisitesofpractice_set", many=True
        )
        self.fields["outcomes"] = OutcomesOfPracticeSerializer(
            source="outcomesofpractice_set", many=True
        )
        self.fields["competences"] = SerializerMethodField()
        self.fields["editors"] = userProfileSerializer(many=True)
        self.fields["practice_in_change_block"] = (
            WorkProgramChangeInDisciplineBlockModuleForWPinFSSerializer(many=True)
        )
        self.fields["expertise_with_practice"] = ShortExpertiseSerializer(
            many=True,
        )
    ...
    def get_competences(self, instance) -> list[dict]:
        ...
        for competence in competences:
            zuns = ZunPractice.objects.filter(
                practice_in_fs__practice__id=instance.id,
