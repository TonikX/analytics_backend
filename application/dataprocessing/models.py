...
class User(AbstractUser):
    ...
    expertise_status_notification = models.BooleanField(
        default=False, blank=True, null=True
    )
    expertise_comments_notification = models.BooleanField(
        default=False, blank=True, null=True
    )
    structural_unit = models.ManyToManyField(
        "workprogramsapp.StructuralUnit",
        through="workprogramsapp.UserStructuralUnit",
        blank=True,
    )
...
class Items(models.Model):
    ...
    domain = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="Укажите область",
        verbose_name="Область знаний",
        on_delete=models.CASCADE,
        related_name="items_in_domain",
    )
    ...
    value = models.IntegerField(
        blank=True, null=True, default=0, verbose_name="Значение"
    )
    source = models.CharField(max_length=200, blank=True, verbose_name="Источник")
    # date_created = models.DateField(auto_now_add = True)
    relation_with_item = models.ManyToManyField(
        "Items", verbose_name="Связи айтима", through="Relation"
    )
...
class Relation(models.Model):
    ...
    item1 = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="item1",
        verbose_name="Элемент РПД",
    )
    relation = models.CharField(
        choices=STATUS_CHOICES, max_length=10, default=HIERARCHY, verbose_name="Связь"
    )
    ...
    count = models.IntegerField(
        null=True, blank=True, default=0, verbose_name="Повторения"
    )
