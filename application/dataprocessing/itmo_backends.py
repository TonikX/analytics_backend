...
class AuthenticateByCodeISU(ListAPIView):
    ...
    def get(self, request):
        ...
        if 'access_token' in obtain_isu:
            ...
            if not user:
                password = self.new_password_with_random(isu_profile["isu"], isu_profile["given_name"])
                ...
                if user.check_password(password):
                    user.password = self.new_password_with_random(isu_profile["isu"], isu_profile["given_name"])
                    user.save()
