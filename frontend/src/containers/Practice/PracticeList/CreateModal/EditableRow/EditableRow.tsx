...
function EditableRow(props: EditableRowProps) {
    ...
    const handleEvaluationToolsChange = (e: React.ChangeEvent<any>) => {
        updateRow({
            ...section,
            //@ts-ignore
            [PracticeFields.EVALUATION_TOOLS]: e?.target?.value,
        }, semesterNum);
    };
        ...
        <TableRow>
            <TableCell className={classes.centerCell}>
                <TextField
                    variant="outlined"
                    size="small"
                    defaultValue={section[PracticeFields.ZE_V_SEM]}
                    className={classes.smallInput}
                    type="number"
                    onChange={handleChangeField(PracticeFields.ZE_V_SEM)}
                />
            </TableCell>
            <TableCell className={classes.centerCell}>
                <Select value={section[PracticeFields.EVALUATION_TOOLS]} placeholder="Оценочное средство" onChange={handleEvaluationToolsChange} fullWidth multiple>
