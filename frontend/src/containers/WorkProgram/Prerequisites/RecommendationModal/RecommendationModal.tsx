...
class RecommendationModal extends React.PureComponent<RecommendationModalProps> {
    ...
    handleSave = () => {
        ...
        selectedItems.forEach((id) => {
            this.props.actions.addPrerequisite({
                id: null,
                masterylevel: '1',
                item: {
                    id
    ...
    render() {
        ...
        const recommendationsContent = recommendations.length === 0 ? <Typography>
            Для вас пока нет рекомендаций. Они появятся позже на основе созданных вами РПД
        </Typography> : (
            ...
            >
                <DialogTitle>
                    <div className={classes.dialogTitle}>
                        Рекомендации пререквизитов
                        <Tooltip
                            title={
                                <span style={{ fontSize: '13px' }}>
                                    Пререквизит - объект, отражающий конкретное знание из конкретной области
